...
import { DeploymentParams, HydraStakeAPI } from "./api.js";
...
import { HydraStakeContractProvider } from "./api.js";

const DEPLOY_OR_JOIN_QUESTION = `
    You can do one of the following:
    1. Deploy a new liquid staking contract
...
const resolve = async (
  ...
  const address =
    "mn_shield-addr_test1stj87fh32cyc04upht5t52aqqayn8424ypkw28sypjp4tdhwkq5sxqzhwqh9hpqj4k3psgg3shwttn00tqx75dxzuzp2vxe49h9wj9suayn670zg";

  switch (choice) {
    case "deploy":
      {
        try {
          api = await HydraStakeAPI.deployHydraStakePool(
            address,
            providers,
            params
          );
      ...
      break;
    case "join": {
      api = await HydraStakeAPI.joinHydraStakePool(
        providers,
        "02009fbf069c9273d80c210230c54c8a3ea534fb0fcfa165c4d87204ffe766e13881"
      );
      console.log(
...
const circuit_main_loop = async (
  ...
  try {
    ...
    while (true) {
      const action = await rli.question(CIRCUIT_MAIN_LOOP_QUESTION);
      switch (action) {
        case "1": {
